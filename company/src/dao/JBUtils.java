...
public class JBUtils
{
	...
	/**自身对象确保唯一**/
	private static JBUtils jdbcUtil= null;
	
	/********************************************************************************/
	
	public static synchronized JBUtils getInitJDBCUtil()
	{	
		configure("mysql","127.0.0.1","3306","root","2246260","myjavadb");
		/**确保JDBCUtil在主方法中不使用new**/
		if(jdbcUtil==null)
		{
		  	jdbcUtil = new JBUtils();
		}
		return jdbcUtil;
	}
	...
	private static void configure(String sql, String address, String port, String username, String password, String dbname)
	{
		...
		JBUtils.sql = sql;
		JBUtils.address = address;
		JBUtils.port = port;
		JBUtils.username = username;
		JBUtils.password = password;
		JBUtils.dbname = dbname;
		
		JBUtils.url="jdbc:"+JBUtils.sql+"://"+JBUtils.address+":"+JBUtils.port+"/"+JBUtils.dbname;
	}
	...
	public Connection getConnection()
	{
		try
		{
			connection=DriverManager.getConnection(JBUtils.url,JBUtils.username,JBUtils.password);
		}
